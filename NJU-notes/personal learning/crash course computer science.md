...
1.  计算机的本质：极其简单的组件，经过一层层复杂的抽象，做出复杂的工作

2. 控制电流的技术发展

继电器→真空管→晶体管 拥有了细微控制电流的手段
计算机最早是机电设备，一般使用十进制计数，使用齿轮与纸带
tip:早期机械计算机，被虫子(bug)卡住引发故障，故现代编程有debug之称；刚开始使用打孔卡片/纸带，对于漏洞(holes)真的是物理意义上的在纸带上打补丁(patch)（笑）

3. 电流可以传递信号，通过电路逻辑门可以参与逻辑运算

二进制：通电1 断电0  易于区分、防干扰，拥有数学布尔代数的现成优势
布尔代数：变量：true/false 可以进行逻辑操作NOT/AND/OR

基于此引出基本的逻辑电路

| NOT 非门  |  AND 与门  |  OR 或门 | XOR 异或门 |
| ---- | ---- | ---- | ---- |
| 真假反转   |   同真为真，一假即假  |  一真为真，同假为假 | 同假异真 |
...
以数字表示各种语言符号的系统：ASCII（7位，有局限）、 Unicode（足以表示世界上所有语言）
...
由基础的门组件抽象成半加器（只能用于一位运算），多个半加器抽象成一个全加器（可用于多位运算，可处理进位）
8 bit 加法（1个半加器+7个全加器= 8-bit ripple adder ）

逻辑单元：

执行逻辑操作及数值验证操作，拥有flag

把算数逻辑单元再次进行抽象，用V表示
OC(operation code)告诉ALU执行什么操作
![[crash course computer science_202601201900.png]]
...
使用逻辑门可抽象成锁存器 *latch*（存储一位），加上允许写入线变为门锁 *Gated Latch*，多个并列的门锁抽象成一个寄存器*register*（可用于多位存储）

使用门锁矩阵有利于统筹管理，对访问和修改的权限进行限制

内存RAM(random access memory,根据地址可以随时访问任何位置) 是由多个存储模块抽象出来的 ，断电即失忆

7. 逻辑门的抽象组合成更高的抽象 ：cpu(RAM+register+ALU)
 ![[crash course computer science_202601201913.png]]

cpu负责执行程序，时钟精准地调控着CPU的运行节奏

cpu工作过程主要有三步：取指令*fetch*，解码*decode*，执行*execute*，完成这三步的速度叫做时钟速度*Clock Speed*（Hz）
超频：加快时钟速度

8. 强大而抽象的控制方式 ：指令与程序

指令集是指令的使用说明，不同指令具有不同简单的功能，多个指令组合成强大的程序，可以去处理复杂的任务。
现代CPU使用两种策略表示更多指令：用更多位表示指令/可变指令长度
...
增加核心数同时执行多个指令流  多核core，多核不够时可用多个cpu

提高时钟频率

指令流水线，即并行处理（需要搞清指令的依赖关系）。现代CPU采用”分支预测“处理jump等条件分支

给CPU加一点RAM（“缓存” *cache* ），解决延迟

dirty bit代表是否需要将缓存写回RAM

10. 早期的编程方式

打孔纸卡 -> 插线板 -> 面板拨开关
A "stored-program computer" is a computer that stores program instructions in electronic memory 
将程序和数据放在一个地方（内存）就是冯诺依曼架构 标志：一个处理器（有ALU）+数据寄存器+指令寄存器+指令地址寄存器+内存（存数据+指令）

11. 编程语言的发展

机器码（机器语言），这是机器唯一理解的东东。

汇编语言：使用汇编器*Assembler*将助记符（如 LOAD A 14）转化为二进制

A-0（已失落）
高级语言:使用编译器*Compiler* 将高级语言转化为低级语言
...
ps：从冗长的main函数到将各个功能封装成不到百行代码的函数，再把功能相近的函数封装成对象，程序的可维护性与可读性大大增强，git等手段使得多人同时编写同一个程序成为可能。

17. 集成电路与摩尔定律

从分立元件*Discrete components* 到集成电路ic(解决了“数字暴政”问题 *Tyranny of Numbers* ) 通过PCB印刷电路板实现

光刻技术的进步使得小型化成为现实

摩尔定律总结了性能和成本的规律，但正在受到光波长，量子效应的挑战

18. 操作系统

os也是一种程序，具有操作硬件的特殊权限，运行管理着其他的程序，充当着软硬件之间的桥梁，提供API来抽象硬件（设备驱动程序 *device drivers* ）
...
19. 存储技术的发展
纸卡 -> 延迟线存储器 -> 磁芯 、磁带、磁鼓 -> 硬盘 、固态硬盘
技术创新使得存储的效率越来越高，考虑到整体的性价比，混合存储最为有利(Memory Hierachy）
...
主要分为无损（游程编码和字典编码）和有损（perceptual coding 依据人类感知的心理模型/ temporal redundancy视频）两大类
霍夫曼树
