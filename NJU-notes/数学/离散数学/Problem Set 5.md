...
$\bigcup_{i=1}^n A_i=A_n$（或者写成 $\{ x \in \mathbb{Z} \mid x \le n \}$）。

#### (b) 求 $\bigcap_{i=1}^n A_i$
*   **CS 直觉**：交集是找所有集合的“共同核心”。
*   **推导**：因为 $A_1$ 是这串序列里最小的，且它被包含在所有的 $A_i$ 中，所以它就是公共部分。
*   **满分答案**：**$A_1$** （或者写成 $\{ x \in \mathbb{Z} \mid x \le 1 \}$）。

---

### 🧬 Problem 9：后继运算（冯·诺依曼序数的构造）
