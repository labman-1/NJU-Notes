...
(c)令$A=\{a|a\in[0,2]\},B=\{a|a\in[0,1]\}$,则A，B均为不可列集合，$A-B=\{a|a\in(1,2]\}$，为不可列集。
# P6
...
# P9
证明：
1. 若B、A-B均为有限集，显然$A=B\cup(A-B)$为有限集，必定为可列集。
2. 若B、A-B中有一个为有限集，另一个为可列无穷集，不妨设B为有限集。可以把B、中的元素排在队伍最前面，然后把A-B的元素接在后面。因为A-B无限，所以这种“平移”操作可以容纳B，$A=(A-B)\cup B$可列。
3. 若A-B、B均为无穷可列集.A-B的第一个元素对应0，第二个元素对应2，第n个元素对应2(n-1)，B的第一个元素对应1，第二个元素对应3，第n个元素对应2n-1.这样就把这两个无限集塞进了一个自然数序列。故$A=B\cup(A-B)$为可列集。
综上，A是可列集。
# P10
证明：要证明可数个可列集的并集亦为可列集等价于证明$有可数个S_i为可列集,\cup^{\infty}_{i=1}S_i$是可列集。既然$S_i$是可列的，可以把每个集合的元素排成一行：
...
# P11
证明：因为$B\approx C$，所以存在双射函数$h:B\to C$。构建函数$g:(A\cup B)\to(A\cup C)$,$$g(x)=\begin{cases}x,x\in A\\ h(x),x\in B\end{cases}$$
...
# P12
证明：
假设该集合是可列的，记各个序列分别为$S_1,S_2,S_3,…$,构造一个新的序列$B=(b_1,b_2,b_3,…)$,规则如下：若$S_i$的第i位数字为1，则$b_i=2$，若$S_i$的第i位数字为2或3，则$b_i=1$。则B与所有已列出的序列$S_i$均不同，矛盾。故该集合不可列。
# P13
先证明$\left|2^{\mathbb{N}}\right|\le\left|[0,1)\right|$，定义函数f，将序列$S=(a_1,a_2,…)$映射为十进制小数:$f(S)=0.a_1a_2a_3…$,由于在十进制中，只有以9为结尾的无限循环小数才会出现重名问题，而序列S中仅有0和1，所以f为单射。
再证明$\left|2^{\mathbb{N}}\right|\ge\left|[0,1)\right|$.任意一个实数都可以写为二进制形式，规定统一不取以全一为结尾的数。则能为每个x找到一个唯一的0-1序列。不同的实数由于大小不同，对应的二进制序列一定不同，这样的一个映射亦为单射。
依据三明治定理，既然双向单射存在，则$2^\mathbb{N}\approx [0,1)$.
$\intt_De^{-(x+y)}dxdy,D=\{(x,y)|x\ge0,y\ge0\}$